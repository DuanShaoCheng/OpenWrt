...
| 文件名 | 设备/用途 | 建议 |
| --- | --- | --- |
| `x86-erofs-uefi.img.gz` | x86/64 UEFI 物理软路由 | 推荐主用，写入 SSD、硬盘或 U 盘 |
| `x86-erofs-bios.img.gz` | x86/64 Legacy BIOS 物理软路由 | 老机器或不支持 UEFI 时使用 |
| `x86-uefi.iso` | x86/64 UEFI ISO | 测试或临时启动，不建议长期主系统 |
| `x86-bios.iso` | x86/64 Legacy BIOS ISO | 测试或临时启动，不建议长期主系统 |
| `r2s-squashfs-sysupgrade.img.gz` | NanoPi R2S | 首次写 TF 卡和 LuCI 后台升级都优先用它 |
| `n1-s905d.img.gz` | 斐讯 N1 | 先写入 U 盘启动，确认正常后再考虑 eMMC |
| `README_RELEASE_ASSETS.txt` | Release 附件说明 | 大文件分卷时看这里的合并方法 |
| `SHA256SUMS.txt` | 校验文件 | 下载后用它核对文件完整性 |
| `x86-BUILD_INFO.txt`、`r2s-BUILD_INFO.txt`、`n1-BUILD_INFO.txt` | 构建信息 | 查看默认 IP、登录信息、分区大小等 |
| `x86-SOURCE_VERSIONS.txt`、`r2s-SOURCE_VERSIONS.txt`、`n1-SOURCE_VERSIONS.txt` | 源码版本 | 查看 OpenWrt、feeds 和第三方插件 commit |
...
7. 从目标盘启动软路由。
8. 访问 `http://10.0.0.1`。
9. 首次启动后等待 1 到 3 分钟，固件会在准备好扩容分区后自动重启一次。
...
6. 首次启动后等待 1 到 3 分钟，固件会在准备好扩容分区后自动重启一次。
...
固件自动重启一次后，/overlay 使用这个 Btrfs 扩容空间
```
...
- 首次启动时会自动安排一次重启；如果电源过早断开，下一次启动会继续尝试。
- Docker 默认数据目录为 `/mnt/data/docker`，服务默认禁用，需要时手动启用。
